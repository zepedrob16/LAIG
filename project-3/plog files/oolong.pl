...
*/
game_loop(Game, SeatIndex, FinalGame) :-
  ...
  play_turn(Game, SeatIndex, FinalGame).
...
game_loop(Game, SeatIndex, FinalGame) :-
  ...
  play_turn(Game, SeatIndex, UpdatedGame), 
...
*/
game_loop(Game, SeatIndex, UpdatedGame) :-
  get_gamemode(Game, Mode),
  Mode = 2,
...
game_loop(Game, SeatIndex, UpdatedGame) :- UpdatedGame = Game.

bot_play_easy(Game, [SeatIndex-UpdatedGame]) :-
  bot_play_turn_easy(Game, UpdatedGame, SeatIndex).

get_score_black(Game, CountB) :- 
  get_tracker(Game, Tracker),
  count(b, Tracker, CountB).

get_score_green(Game, CountG) :-
  get_tracker(Game, Tracker),
...
play_turn(Game, SeatIndex, UpdatedGame) :- UpdatedGame = Game.
