 ...
 */
 function XMLscene(interface) {
    ...
    this.last_picked_teacup;
    this.change_camera = 0;
    this.timer = 0;

    this.switch_root = true;
...
XMLscene.prototype.update = function(tick) {
    ...
    // Turn timer.
    if (this.change_camera == 1 && gamestate.turn == 'g'){
        this.graph.flip_camera_player();
    }

    if (this.change_camera == 1 && gamestate.turn == 'b'){
        this.graph.reverse_flip_camera();
    }

    if (this.last_second != Math.round(this.last_tick / 1000)) {
        increment_second();
        this.last_second = Math.round(this.last_tick / 1000);
    }
...
XMLscene.prototype.handle_pick = function () {
    ...
    if (!this.pickMode && this.pickResults != null & this.pickResults.length > 0) {
        for (let i = 0; i < this.pickResults.length; i++) {
            ...
            if (this.pickResults[i][1] >= 97 && this.pickResults[i][1] <= 176){

                this.last_picked_teacup = picking_hub(this.pickResults[i][1]);
            }
            else
                this.last_picked_seat = picking_hub(this.pickResults[i][1]);
 ...
 */
 XMLscene.prototype.display = function() {
    ...
    if (this.last_picked_teacup != undefined && this.last_picked_teacup.hasOwnProperty('animations')) {
        if (this.last_picked_teacup.animations[0].completed) {

            if (gamestate.changed == 1) {
                this.graph.move_teacup(this.last_picked_seat, gamestate.turn, teacup_id);
                this.change_camera = 1;
                gamestate.changed = 0;
                gamestate.turn_elapsed = 0;
            ...
            this.last_picked_teacup = undefined;
        }
    }
