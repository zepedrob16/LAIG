...
MySceneGraph.prototype.parseNodes = function(nodesNode) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        if ((nodeName = children[i].nodeName) == "ROOT") {
            ...
            if (this.reader.hasAttribute(nodeSpecs[materialIndex], 'invert')) {
              var invert_rgb = this.reader.getBoolean(nodeSpecs[materialIndex], 'invert');

              if (invert_rgb) {
                for (let i = 0; i < 3; i++) {
                  this.materials[materialID]['ambient'][i] = 1 - this.materials[materialID]['ambient'][i];
                  this.materials[materialID]['diffuse'][i] = 1 - this.materials[materialID]['diffuse'][i];
                  this.materials[materialID]['emission'][i] = 1 - this.materials[materialID]['emission'][i];
                  this.materials[materialID]['specular'][i] = 1 - this.materials[materialID]['specular'][i];
                }
              }
            }
            ...
            var type = this.reader.getItem(descendants[j], 'type', ['rectangle', 'cylinder', 'sphere', 'triangle', 'circle', 'patch', 'tree']);
...
MySceneGraph.prototype.displayScene = function() {
  this.processNode(this.idRoot, this.nodes[this.idRoot].textureID, this.nodes[this.idRoot].materialID);
}

MySceneGraph.prototype.processNode = function(root_node_id, init_texture, init_material) {

  let root_node = this.nodes[root_node_id]; // The root node object.
  var new_material = init_material, new_texture = init_texture;
  ...
  for (let i = 0; i < root_node.children.length; i++) {
    ...
    this.processNode(root_node.children[i], new_texture, new_material);
