...
function MyGraphLeaf(graph, xmlelem) {
  ...
  var type = this.graph.reader.getItem(xmlelem, 'type', ['rectangle', 'cylinder', 'sphere', 'triangle', 'circle', 'patch', 'diamond', 'square']);

  if (this.graph.reader.hasAttribute(xmlelem, 'args')) {
  ...
  if (type == 'rectangle') {
    ...
    this.primitive = new Diamond(this.graph.scene, args[0]);
  }
  else if (type == 'square') {
    this.primitive = new Square(this.graph.scene, this.graph.reader.getFloat(xmlelem, 'texangle'));
  }
  return;
