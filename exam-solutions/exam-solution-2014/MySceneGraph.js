...
MySceneGraph.prototype.parseNodes = function(nodesNode) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        if ((nodeName = children[i].nodeName) == "ROOT") {
            ...
            if (this.reader.hasAttribute(nodeSpecs[materialIndex], 'force')) {
              var force_attribute = this.reader.getBoolean(nodeSpecs[materialIndex], 'force');
              this.nodes[nodeID].force = force_attribute;
            }
            ...
            var type = this.reader.getItem(descendants[j], 'type', ['rectangle', 'cylinder', 'sphere', 'triangle', 'circle', 'patch', 'diamond', 'square']);
...
MySceneGraph.prototype.displayScene = function() {
  this.processNode(this.idRoot, this.nodes[this.idRoot].textureID, this.nodes[this.idRoot].materialID, false);
}

MySceneGraph.prototype.processNode = function(root_node_id, init_texture, init_material, forced) {
  ...
  for (let i = 0; i < root_node.children.length; i++) {
    ...
    let force_value = this.nodes[root_node_id].force && !forced;

    if ((force_value == undefined || !force_value) && child.materialID != 'null') {
    ...
    this.processNode(root_node.children[i], new_texture, new_material, force_value);
